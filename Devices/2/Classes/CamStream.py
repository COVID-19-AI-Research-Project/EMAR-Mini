...
class CamStream(Thread):
    ...
    CamStream function for EMAR Emergency Assistance Robot.
    """
    ...
    def run(self):
        ...
        try:
            # Starts web server
            server = ThreadedHTTPServer((self.Helpers.confs["EMAR"]["ip"], self.Helpers.confs["TASS"]["server"]["port"]), CamHandler)
            self.Helpers.logger.info("Stream server started on http://" + self.Helpers.confs["EMAR"]["ip"] + ":" + str(self.Helpers.confs["TASS"]["server"]["port"]))
